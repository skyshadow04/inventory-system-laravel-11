            ...
            (function () {
                ...
                const formTemplates = {
                    ...
                    'Electrical': {
                        ...
                        fields: [
                            { name: 'sr_number_elec[]', label: 'SR# (Item ID)', type: 'text', readonly: true, disabled: true, placeholder: 'Auto-generated on save' },
                            { name: 'item_description_elec[]', label: 'Item Description', type: 'text', required: true, placeholder: 'e.g. Cable or Light Fixture' },
                            { name: 'category_name_elec[]', label: 'Category', type: 'text', placeholder: 'e.g. Electrical Supplies' },
                            { name: 'make_elec[]', label: 'Make', type: 'text', placeholder: 'e.g. Brand Name' },
                            { name: 'venue_elec[]', label: 'Venue', type: 'text', placeholder: 'e.g. Distribution Room' },
                            { name: 'barcode_elec[]', label: 'Barcode', type: 'text', placeholder: 'e.g. 123456789' },
                            { name: 'quantity_in_hand_elec[]', label: 'Quantity in Hand', type: 'number', step: '0.01', placeholder: '0' },
                            { name: 'physical_stock_elec[]', label: 'Physical Stock', type: 'number', required: true, step: '0.01', placeholder: '0' },
                            { name: 'remarks_elec[]', label: 'Remarks', type: 'textarea', placeholder: 'Additional notes...' }
                        ]
                    },
                    'ENGG / MEC': {
                ...
                function createFieldHTML(field) {
                    ...
                    const disabledClass = field.disabled ? ' bg-gray-100 text-gray-500 cursor-not-allowed' : '';
                    ...
                    if (field.type === 'textarea') {
                        return `
                            <div class="md:col-span-2 lg:col-span-3 xl:col-span-4">
                                ...
                                <textarea name="${field.name}" rows="2"${isRequired}${isReadonly}${isDisabled} class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm${readonlyClass}${disabledClass}" placeholder="${field.placeholder}"></textarea>
                            </div>
                        `;
                    } else {
                        ...
                        return `
                            <div>
                                <label class="block text-sm font-medium text-gray-700">${field.label}${requiredMark}</label>
                                <input type="${field.type}" name="${field.name}"${isRequired}${step}${isReadonly}${isDisabled} min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm${readonlyClass}${disabledClass}" placeholder="${field.placeholder}" />
                            </div>
                        `;
                    }
                }
                ...
                function createRow() {
                    ...
                    const srField = row.querySelector('[name="sr_number[]"], [name="sr_number_eng[]"], [name="sr_number_elec[]"], [name="sr_no_mech[]"], [name="sr_no_ops[]"]');
                    if (srField) {
